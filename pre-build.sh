...
wget ${URL}${VERSION}/LexFloatClient-Linux.zip
...
unzip LexFloatClient-Linux.zip -d ~/android
...
cp ~/android/libs/clang/arm64-v8a/libLexActivator.so lexfloatclient/src/main/jniLibs/arm64-v8a
cp ~/android/libs/clang/armeabi-v7a/libLexActivator.so lexfloatclient/src/main/jniLibs/armeabi-v7a
